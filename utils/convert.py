...
def get_flat_ids(ids_vec,dim,n_points):
    ...
    # initialize array to get multi index
    arr = np.zeros((2,n_ids*dim))
    # row index is in ids_vec
    arr[0,:] = np.repeat(ids_vec, dim)
    # col index is across dimension
    if(dim==3): 
...
def array_filter_id_nan(arr,ids):
    # puts array into order of ids but is able to handle nan in ids
...
def numpy2PointCloud(coor):
    ...
    #necessary for 2d arrays with open3d
    if(coor.shape[1]==2):
